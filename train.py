...
from utils.dataset import train_dataloader, val_dataloader, test_dataloader, Noise
...
if __name__ == '__main__':
    ...
    if c.dataset["noise_csv"] is not None:
        ap.setNoise(Noise(c.dataset["noise_csv"]))
    ...
    if (checkpoint_path is not None):
        ...
        epoch = load_checkpoint(os.path.abspath(checkpoint_path), model,
                                optimizer, device)
    else:
        epoch = 0
    ...
    while epoch < c.train_config['epochs']:
        ...
        if epoch % c.train_config["summary_interval"] == 0:
            ...
            print(
                f'Avg. Val Loss: {val_loss:>8f} / Val std. dev: {val_std:>0.8}\n'
            )
        ...
        if val_loss < best_saved_loss:
            ...
            save_checkpoint(best_checkpoint_path, model, optimizer, epoch,
                            val_loss)
